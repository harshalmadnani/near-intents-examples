...
// Configuration
const senderAccount = "your-account.near"; // Configure in .env
const senderPrivateKey = process.env.SENDER_PRIVATE_KEY as string; // Configure in .env
const recipientAddress = '0x553e771500f2d7529079918F93d86C0a845B540b' // Destination address on Arbitrum
...
async function fullSwap() {
  try {
    ...
    console.log("--------------------------------");
    const quote = await getQuote(false, senderAccount, recipientAddress, originAsset, destinationAsset, amount);
    ...
    console.log("--------------------------------");
    const depositResult = await sendTokens(senderAccount, senderPrivateKey, depositAddress, amount);
